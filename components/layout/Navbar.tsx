          ...
          <div className="flex justify-center lg:w-1/3">
            <Link href="/" aria-label="HOOR Home" className="group">
              <div className="relative w-14 h-14 lg:w-[60px] lg:h-[60px] transition-transform duration-700 group-hover:scale-105">
                <Image src="/images/logo.png" alt="HOOR" fill sizes="60px" className="object-contain" priority />
              </div>
            </Link>
          </div>
...
function CartButton({ count, onClick }: { count: number; onClick: () => void }) {
  return (
    <button onClick={onClick} aria-label={`Open cart (${count} items)`}
      className="relative text-onyx/60 hover:text-gold transition-colors duration-300">
      <ShoppingBag size={18} />
      {count > 0 && (
        <motion.span
          key={count}
          ...
          className="absolute -top-2 -right-2 w-4 h-4 bg-gold text-ivory text-[9px] font-body flex items-center justify-center rounded-full">
          {count > 9 ? '9+' : count}
        </motion.span>
      )}
    </button>
