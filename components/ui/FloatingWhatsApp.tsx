    ...
    <AnimatePresence>
      {show && (
        ...
        >
          <AnimatePresence>
            {tip && (
              <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
                ...
                <p className="font-body text-[10px] tracking-ultra uppercase text-onyx whitespace-nowrap">Chat with Shiza</p>
                <p className="font-body text-xs text-warm-light mt-0.5">Replies personally & promptly</p>
              </motion.div>
            )}
          </AnimatePresence>

          <motion.a
            href={`${BRAND.whatsapp}?text=${encodeURIComponent(BRAND.whatsappMsg)}`}
          ...
          >
            <motion.span animate={{ scale: [1, 1.5, 1], opacity: [0.3, 0, 0.3] }}
